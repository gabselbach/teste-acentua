...
def Verbos(palavra,retorno):
  ...
  pattern = '^(.*).[amamos|ssemos|eis|reis|sseis|rao|ara]$'
  ...
  if result:
    ...
    if(oxi.search(palavra)):
      ...
      result = re.split('amos|ssemos|eis|reis|sseis|ara', palavra)
      if(oxi.search(palavra)):
        forte = ' '.join(result[1])
      else:
...
def processamento():
  ...
  for j in range(len(retorno)):
    if(len(retorno[j]['PALAVRAANT'])<=4 and retorno[j]['PALAVRAVOP']=='NAN' ):
      ...
      if(retorno[j]['PALAVRAVOP']=='NAN'):
        ...
        if(retorno[j]['REGRAVERB']==0 and retorno[j]['REGRANaoVERB']==0 and retorno[j]['ACENTO']!=0):
          ...
          ex.append(retorno[j].copy())
      else:
        retorno[j]['REGRAVERB'] =  Verbos(retorno[j]['PALAVRAANT'],retorno[j])
        retorno[j]['REGRANaoVERB'] = terminacao(retorno[j]['PALAVRAANT'],retorno[j]['CLASSE'])
  final=time.time()
  t = final-inicio
  return render_template('mostraconteudo.html',titulo = titulo,texto = retorno,t=t,ex=ex)